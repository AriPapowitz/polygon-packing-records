...
# area lower bound: container circumradius can never go below this
# (reduces to sqrt(N) when inner and container shapes are the same)
unit_polygon_area = nsi / 2 * np.sin(2 * np.pi / nsi)
unit_container_area = nsc / 2 * np.sin(2 * np.pi / nsc)
area_lower_bound_S = np.sqrt(N * unit_polygon_area / unit_container_area)
...
def repetition(seed):
    ...
    np.random.seed(seed)
    dynamic_S = area_lower_bound_S * (2 + np.random.rand() * 2)
    initial_S = dynamic_S
...
stem = f"{N}_{nsi}_in_{nsc}"
if os.path.exists(f"{stem}.{args.output_format}") or os.path.exists(f"{stem}.json"):
    file_i = 1
    while os.path.exists(f"{stem}_({file_i}).{args.output_format}") or os.path.exists(f"{stem}_({file_i}).json"):
        file_i += 1
    stem = f"{stem}_({file_i})"

if args.output_format in ("png", "svg"):
    ppt.savefig(f"{stem}.{args.output_format}")
...
with open(f"{stem}.json", "w") as f:
    json.dump({
        ...
        "inner_sides": nsi,
        "container_sides": nsc,
        "container_circumradius": best_S,
        "side_length": best_S * np.sin(np.pi / nsc) / np.sin(np.pi / nsi),
        "placements": [
            {"x": best_values[i * 3], "y": best_values[i * 3 + 1], "angle": best_values[i * 3 + 2]}
            for i in range(N)
        ],
    }, f, indent=2)
